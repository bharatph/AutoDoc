...
int sh_process(int jlen, job *jobs, node::Node *n, const char *line_c) {
  int line_len = strlen(line_c);
  char *line = (char *)malloc(line_len);
  strncpy(line, line_c, line_len);
  int count = -1;
...
int sh_next(int jlen, job *jobs, const char *_shell_name) {
  ...
  while (1) {
    ...
    if (position == bufsize) {
      bufsize += SH_BUFSIZE;
      buffer = (char *)realloc(
          buffer, sizeof(char) * bufsize); // TODO replace with calloc + realloc
      if (!buffer)
        log_fat(_SH_TAG, "allocation error, not enough memory...closing");
    }
  }
...
int start_shell(int jlen, job jobs[], const char *_shell_name,
                int exit_code = 100) {
  int status = 0;
...
int sh_help(int jlen, job *jbs, const char *opt = "\0") {
  for (int i = 0; i < jlen; i++) {
    ...
    for (int j = 0; j < jbs[i].opt_length; j++) {
      printf("  |--%s[-%c] - %s\n", jbs[i].options[j].word,
             jbs[i].options[j].letter, jbs[i].options[j].description);
    }
  }
  return 0;
}
